...
describe User do
  ...
  it "asserts database counts" do
    ...
    assert_equal 0,   Track.where(rdio_name: nil).count
    assert_equal 0,   Track.where(rdio_artist: nil).count
    assert_equal 0,   Track.where(rdio_album: nil).count
    assert_equal 0,   Track.where(rdio_duration: nil).count
    assert_equal 0,   Track.where(rdio_isrcs: nil).count

    assert_equal 424, Track.where(spotify_id: nil).count
    assert_equal 424, Track.where(spotify_name: nil).count
    assert_equal 424, Track.where(spotify_artist: nil).count
    assert_equal 424, Track.where(spotify_album: nil).count
    assert_equal 424, Track.where(spotify_duration_ms: nil).count
    assert_equal 424, Track.where(spotify_isrcs: nil).count

    assert_equal 424, Track.where(isrc: nil).count

    assert_equal 0,   Track.db["SELECT * FROM tracks WHERE array_length(rdio_isrcs,1) = 0"].count
    assert_equal 399, Track.db["SELECT * FROM tracks WHERE array_length(rdio_isrcs,1) = 1"].count
    assert_equal 24,  Track.db["SELECT * FROM tracks WHERE array_length(rdio_isrcs,1) = 2"].count
    assert_equal 0,   Track.db["SELECT * FROM tracks WHERE array_length(rdio_isrcs,1) = 3"].count
    assert_equal 1,   Track.db["SELECT * FROM tracks WHERE array_length(rdio_isrcs,1) = 4"].count
  end
  ...
  it "uses isrcs to find and match spotify tracks" do
    ...
    matched = Track.where("spotify_id IS NOT NULL").order(:rdio_key)
    assert_equal(
      ...
      matched.map(:rdio_key)
    )

    unmatched = Track.where(spotify_id: nil).order(:rdio_key)
    assert_equal(
